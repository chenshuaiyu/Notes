...
因为Android的UI空间不是线程安全的，如果在多线程中并发访问可能会导致UI控制处于不可预期的状态。
...
Thread是一个线程内部的数据存储类，通过它可以在指定的线程中存储数据，数据存储以后，只能在指定的线程中可以获取到存储的数据，对于其他线程来说则无法获取到数据。
