...
2. **Q**：volatile作用是什么？**A**：禁止指令重排序。`instance = new LazySingleton()`分为三步，1.分配内存空间，2.初始化，3.引用指向该内存空间。如果不加volatile，2、3步之间发生指令重排序，3先执行，使得`instance!=null`，其他线程在第一层if时，即可返回instance，但是这时未进行初始化。
